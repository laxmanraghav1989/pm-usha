...
@Component({
  selector: 'cfs-view-meru-univer',
  templateUrl: './view-meru-univer.component.html',
  styleUrls: ['./view-meru-univer.component.scss']
})
export class ViewMeruUniverComponent implements OnInit {
...
findNpd(month, year) {
  if (month && year){
    // const d = new Date()
    // let updateMonth = "" + (d.getMonth() + 1);
    // if (this.year === '2025') {
    //   if (this.month >= updateMonth) {
    //   this.isView = false;
    //     this.notification.showValidationMessage("Data for this month can be updated later ONLY!");
    //     return
    //   }
    // }

      // Your existing logic if submission is allowed
          this.isView = true;
          this.closeAllPanels();
          this.getLock1();
  } else {
...
// }
  ...
  lockTag() {
    ...
    this.tabValueAllTag()
        ...
        // } else if (this.softconditions?.length !== 0) {
        //   this.notification.showValidationMessage("Please fill-in the details in 'Soft Component'!");
        // }  else {
...
getLock1() {
  ...
  // if (this.year && this.month && this.stateCode) {
...
exportTagDataToExcel() {
      ...
      const cleanedData = this.infraConstructionList.map((x,i) => ({
        ...
        'Project Tag': this.getTagNames(x.itemTaggingDetails),
...
getFiniacialProgress$() {
  return new Observable(observer => {
    ...
    this.getFiniacialProgress(); // original function

    let int = setInterval(() => {
      if (this.updateProgressData !== undefined) {
        clearInterval(int);
        observer.next(this.updateProgressData);
        observer.complete();
      }
    }, 150);
  });
...
getInfraCons$(propId: any) {
  return new Observable(observer => {
    ...
    this.getInfraCons(propId);

    let int = setInterval(() => {
      if (this.infraConstructionList !== undefined) {
        clearInterval(int);
        observer.next(this.infraConstructionList);
        observer.complete();
      }
    }, 150);
  });
...
getRenovated$(propId: any) {
  return new Observable(observer => {

    this.getRenovated(propId);

    let int = setInterval(() => {
      if (this.renovatedList !== undefined) {
        clearInterval(int);
        observer.next(this.renovatedList);
        observer.complete();
      }
    }, 150);
  });
...
getEquipment$(propId: any) {
  return new Observable(observer => {

    this.getEquipment(propId);

    let int = setInterval(() => {
      if (this.equipmentList !== undefined) {
        clearInterval(int);
        observer.next(this.equipmentList);
        observer.complete();
      }
    }, 150);
  });
}
getSoftCompoenent$(propId: any) {
  return new Observable(observer => {

    this.getSoftCompoenent(propId);

    let int = setInterval(() => {
      if (this.softComponentList !== undefined) {
        clearInterval(int);
        observer.next(this.softComponentList);
        observer.complete();
      }
    }, 150);
  });
...
getProposedCourseData$(propId: any) {
  return new Observable(observer => {

    this.getProposedCourseData(propId);

    let int = setInterval(() => {
      if (this.courseData !== undefined) {
        clearInterval(int);
        observer.next(this.courseData);
        observer.complete();
      }
    }, 150);
  });
...
getActivitiesData$(propId: any) {
  return new Observable(observer => {

    this.getActivitiesData(propId);

    let int = setInterval(() => {
      if (this.activityData !== undefined) {
        clearInterval(int);
        observer.next(this.activityData);
        observer.complete();
      }
    }, 150);
  });
...
getOutComeIndicator$(propId: any) {
  return new Observable(observer => {
    ...
    this.getOutComeIndicator(propId);

    let int = setInterval(() => {
      if (this.getpropOutcomeData !== undefined) {
        clearInterval(int);
        observer.next(this.getpropOutcomeData);
        observer.complete();
      }
    }, 150);
  });
...
triggerFileInput(item: any) {
  const input = document.getElementById('fileInput-' + item.id) as HTMLElement;
  if (input) {
    input.click();
  }
}
...
onFileSelected(event: any, item: any) {
  ...
  if (file) {
    ...
    // this.apiService.uploadInfraFile(item.id, file).subscribe(...)
  }
}
...
isPreviousData(item: any): boolean {
...
isPreviousMonth(item: any): boolean {
  ...
  const itemYear = Number(item?.prevYear);
  const itemMonth = Number(item?.previousMonth);
...
isSameValue(current: any, previous: any): boolean {
