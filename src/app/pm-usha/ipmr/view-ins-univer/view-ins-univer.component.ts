...
@Component({
  selector: 'cfs-view-ins-univer',
  templateUrl: './view-ins-univer.component.html',
  styleUrls: ['./view-ins-univer.component.scss']
})
export class ViewInsUniverComponent implements OnInit {
...
tabValueAll() {
  ...
  back(){
    sessionStorage.setItem('back','true')
      this.router.navigate([this.routers.viewIpmr])
  }
  ...
  this.getActivitiesDataDownload(12).subscribe(() => {
    if (this.activityData?.length) {
      ...
      this.excelService.exportToExcel(custom_data, 'Activity_Report');
    } else {
...
getProposedOutcomeData$(propId: any) {
  return new Observable(observer => {
