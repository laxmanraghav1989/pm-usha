...
@Component({
  selector: 'cfs-outcome-view-equity',
  templateUrl: './outcome-view-equity.component.html',
  styleUrls: ['./outcome-view-equity.component.scss']
})
export class OutcomeViewEquityComponent implements OnInit {
...
myFilesNameDpr: any;
  ...
  constructor(public api: ApiService, public common: Common, public sharedService: SharedService, public router: Router,
    ...
    this.districtCode = this.route.snapshot.paramMap.get('districtCode')
    this.aisheCode = this.route.snapshot.paramMap.get('aisheCode')
    this.reportDisctrictCode = this.route.snapshot.paramMap.get('districtCode');
  ...
  getProposedOutcomeData(propId) {
    ...
    this.api.getOutComeGenderEquityRevision(this.districtCode, this.componentId, this.sharedService.revAddId)
      .pipe(
        tap(res => {
  ...
  getOtherInfData() {
    this.getpmService.getGenOtherInformation(this.reportDisctrictCode, this.componentId).subscribe((res) => {
