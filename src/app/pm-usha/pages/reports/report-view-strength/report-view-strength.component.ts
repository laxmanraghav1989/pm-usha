...
@Component({
  selector: 'cfs-report-view-strength',
  templateUrl: './report-view-strength.component.html',
  styleUrls: ['./report-view-strength.component.scss']
})
export class ReportViewStrengthComponent implements OnInit {
  ...
  getOtherInfData() {
    this.getpmService.getOtherInformation(this.aisheCode, this.componentId).subscribe((res) => {
      ...
      this.newDprfileId = this.otherInformData.newDprFileId
      if (res.data && res.data.length) {
  ...
  back(){
    this.cachedData.clearData('TagData');
