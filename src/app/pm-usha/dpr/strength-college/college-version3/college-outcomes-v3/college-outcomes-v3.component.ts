...
@Component({
  selector: 'cfs-college-outcomes-v3',
  templateUrl: './college-outcomes-v3.component.html',
  styleUrls: ['./college-outcomes-v3.component.scss'],
  providers: [
    { provide: DateAdapter, useClass: AppDateAdapter },
    { provide: MAT_DATE_FORMATS, useValue: APP_DATE_FORMATS }
  ],
})
export class CollegeOutcomesV3Component implements OnInit {
 ...
 SampleForm: any;
  ...
  constructor(
    ...
    private route: ActivatedRoute, private encrypt: EncryptDecrypt
  ) {
  ...
  saveOtherSourceBoolean() {
    ...
    let payload={
      ...
      "menu":this.insType === 'U' ? this.common.strengthUnivOtherSource : this.common.strengthClgOtherSource
    }
  ...
  getOutComeData() {
    if (this.paramId === this.sharedService.revPrposal) {
      this.getService.getOutComeRevision(this.aisheCode, this.instituteCategory, this.insType === 'U' ? this.sharedService.universityComponentId : this.sharedService.collegeComponentId, this.districtCode, "1").subscribe((res) => {
      ...
      this.getService.getOutCome(this.aisheCode, this.instituteCategory, this.insType === 'U' ? this.sharedService.universityComponentId : this.sharedService.collegeComponentId).subscribe((res) => {
        this.outComeDataList = res.data
          this.dataOutComeList.forEach(element => {
            this.outComeDataList.forEach(e=>{
              if (e.outcomeIndicatorId === element.id) {
  ...
  saveOutCome() {
    ...
    this.api.postOutCome(temp, this.insType === 'U' ? this.common.strengthUnivOutCome : this.common.strengthClgOutCome).subscribe(
      (res) => {
        if (res.status === 200) {
          this.notification.showSuccess();
          this.getOutComeData();
  ...
  add() {
    this.otherSource.push({
      ...
      "amount": 0,
      "componentId": this.insType === 'U' ? this.sharedService.universityComponentId : this.sharedService.collegeComponentId,
      // "componentName": 'Grants to Strengthen Colleges (Accredited & Unaccredited Colleges)',
      "districtCode": this.districtCode,
      "id": 0,
      "instituteCategory": this.insType,
      "scheme": "",
  ...
  submit() {
    ...
    this.api.collegeUnivOtherSource(this.otherSource, this.undertakingNoDuplicationExpenditure, this.insType === 'U' ? this.common.strengthUnivOtherSource : this.common.strengthClgOtherSource).subscribe(res => {
      if (res.status === 200) {
        this.notification.showSuccess()
        this.getOtherSource()
  ...
  getOtherSource() {
    this.getService.getOtherSourceList(this.aisheCode, this.insType === 'U' ? this.sharedService.universityComponentId : this.sharedService.collegeComponentId).subscribe(res => {
  ...
  getOtherInfo() {
    this.api.getOtherInfoDetails(this.aisheCode, this.insType === 'U' ? this.sharedService.universityComponentId : this.sharedService.collegeComponentId).subscribe(res => {   
      ...
      if (res.data && res.data.length) {
        ...
        this.documentOfDpr = res.data['0'].newDprFileName
  ...
  regex(event, reg, value) {
    let a = new RegExp(reg)
    parseInt(value)
    let result: boolean = a.test(value);
    if (!result) {
      // event.preventDefault();
    }
  ...
  submitDoc() {
    ...
    this.api.saveDocumentOtherInfo(payload, this.insType === 'U' ? this.common.strengthUnivOtherInfo : this.common.strengthClgOtherInfo, formdata).subscribe(res => {
      if (res.status === 200) {
        this.notification.showSuccess()
        this.getOtherInfo()
      }
