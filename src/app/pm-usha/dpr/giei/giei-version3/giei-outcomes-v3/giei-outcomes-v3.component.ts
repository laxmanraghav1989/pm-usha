...
@Component({
  selector: 'cfs-giei-outcomes-v3',
  templateUrl: './giei-outcomes-v3.component.html',
  styleUrls: ['./giei-outcomes-v3.component.scss']
})
export class GieiOutcomesV3Component implements OnInit {
 ...
 // aisheCode: string;
  ...
  constructor(
    ...
    private route: ActivatedRoute, private router: Router, private encrypt: EncryptDecrypt
  ...
  deleteDoc(){
    ...
    if (this.myFilesNameDPR1) {
      ...
      if (this.changeDocDPR) {
        ...
        this.api.saveDocumentOtherInfoDPREquityV3(payload, this.common.genderEquityOtherInformation, formdata).subscribe(res => {
          if (res.status === 200) {
            this.notification.showSuccess()
            this.getDocumentsV3();
          }
