...
@Component({
  selector: 'cfs-view-strength-revision',
  templateUrl: './view-strength-revision.component.html',
  styleUrls: ['./view-strength-revision.component.scss']
})
export class ViewStrengthRevisionComponent implements OnInit {
  ...
  constructor(public api: ApiService, public dialog: MatDialog, public common: Common, public sharedService: SharedService, public router: Router,
    ...
    this.componentIdCost = parseInt(sessionStorage.getItem("componentIdV"));

    this.aisheCode = sessionStorage.getItem("aisheCode");
    this.aisheCodeCost = sessionStorage.getItem('userName')
  ...
  getCostListRevision() {
    this.getService.getProposalCostRevision( this.aisheCode , this.id, this.sharedService.revAddId).subscribe(res => {
      if (res && res.length) {
        this.costList = res;
      ...
      .pipe(
        tap(res => {
          ...
          if (this.appId){
              ...
              this.softComponentList = this.softComponentList.filter(item =>
              // ❌ hide deleted
              !(item.recordStatus?.id === 2) &&
              // ❌ hide existing (old v2 only)
              !(item.activeStatus == false && item.recordStatus?.id === 3 && (item.v3 === null || item.v3 === false))
              ...
              this.softComponentList = [...this.existingRecordFilter, ...this.newFilterArr, ...this.newArray, ...this.deleteFilterArr];
              this.softComponentList = this.softComponentList;
          }
          // this.softComponentList = [...this.existingRecordFilter, ...this.newFilterArr, ...this.newArray, ...this.deleteFilterArr];
  ...
  // getDataFinancialEstimate(propId) {
  //   this.api.getFinancialEstimate(this.aisheCode, this.componentId, null, "1").subscribe((res) => {
