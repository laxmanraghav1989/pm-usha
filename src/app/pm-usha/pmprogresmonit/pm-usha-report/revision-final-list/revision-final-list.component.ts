...
@Component({
  selector: 'cfs-revision-final-list',
  templateUrl: './revision-final-list.component.html',
  styleUrls: ['./revision-final-list.component.scss']
})
export class RevisionFinalListComponent implements OnInit {
  ...
  constructor(public api: ApiService, public common: Common, public notification: NotificationService, public masterService: MasterService,
    public sharedService: SharedService, public router: Router, private route: ActivatedRoute, public getService: GetService, public postService: PostService, public encrypt: EncryptDecrypt, private excelService: ExcelService) {
  ...
  ngOnInit(): void {
    ...
    if (sessionStorage.getItem('back') === 'true') {
      ...
      this.reRevisionFlag = sessionStorage.getItem('reRevisionStatus') ? sessionStorage.getItem('reRevisionStatus') : 'ALL'
    ...
    return items.filter((item: any) => {
      if (this.searchText.trim() === '') {
        ...
        return (item.instituteName?.trim().toLowerCase().includes(this.searchText.trim().toLowerCase()))
          ...
          || (item.proposalRevisionFinalSubmissionDateTimeInString?.toString().trim().toLowerCase().includes(this.searchText.trim().toLowerCase()))
  ...
  datailsList(ele: any): void {
    ...
    sessionStorage.setItem('filteredComponentId', this.componentId)
    sessionStorage.setItem('reRevisionStatus', this.reRevisionFlag)
    const isV3ForwardedToNpd = ele.isV3ForwardedToNpd;
      if ((ele.componentId == this.sharedService.collegeComponentId || ele.componentId == this.sharedService.universityComponentId) && !isV3ForwardedToNpd) {
        this.router.navigate([this.routers.viewStrengthenRevisionApp, ele.componentId, this.sharedService.finApproveLIst])
      }
      else if ((ele.componentId == this.sharedService.meruComponentId) && !isV3ForwardedToNpd) {
        this.router.navigate([this.routers.viewMeruRevisionApp, ele.componentId, this.sharedService.finApproveLIst])
      }
      else if ((ele.componentId == this.sharedService.nmdcComponentId) && !isV3ForwardedToNpd){
        this.router.navigate([this.routers.viewNMDCRevisionApp, ele.componentId, this.sharedService.finApproveLIst])
      }
      else if ((ele.componentId == this.sharedService.genderComponentId) && !isV3ForwardedToNpd) {
        this.router.navigate([this.routers.viewEquityRevisionApp, ele.componentId, this.sharedService.finApproveLIst])
      }
      else if(ele.componentId == this.sharedService.genderComponentId && isV3ForwardedToNpd){
        this.router.navigate([this.routers.viewEquityNMDCRevisionAppV3, ele.componentId, this.sharedService.finApproveLIst])
      }
      else if ((ele.componentId == this.sharedService.meruComponentId || ele.componentId == this.sharedService.collegeComponentId || ele.componentId == this.sharedService.universityComponentId) && isV3ForwardedToNpd){
        this.router.navigate([this.routers.viewMeruCollegeUniversityRevisionAppV3, ele.componentId, this.sharedService.finApproveLIst])
      }   
  }
  ...
  collegeList() {
    ...
    this.getService.getfinalSubmitProposal(payload).subscribe((res) => {
      ...
      if ((this.userTypeId === this.sharedService.userTypeList['0'].id || this.userTypeId === this.sharedService.userTypeList['6'].id || this.userTypeId === this.sharedService.userTypeList['7'].id) && this.revision === this.sharedService.finApproveLIst) {
      ...
      if (this.collegeListData.length != 0) {
          ...
          if (this.componentId == this.sharedService.collegeComponentId) {
            let custom_data = this.collegeListData.map((item, index) => ({
              ...
              'Is Re-Revised': item.revisedProposalForwardedtoNpd && item.isV3ForwardedToNpd && item.revisedProposalV3Cost ? 'Yes' : 'No',
              'Total Cost': (item.revisedProposalForwardedtoNpd && item.isV3ForwardedToNpd
                                            ? +item.revisedProposalV3Cost
                                            : +item.revisedProposalForwardedtoNpd
                                                ? +item.revisedTotalCost
                                                : +item.totalCost),
            }))
             let fileNamePrefix = '';
              if (this.componentId == this.sharedService.collegeComponentId) {
                fileNamePrefix = 'Proposal Revision Monitoring Report of Strengthen College';
              } else if (this.componentId == this.sharedService.universityComponentId) {
                fileNamePrefix = 'Proposal Revision Monitoring Report of Strengthen University';
              } else if (this.componentId == this.sharedService.meruComponentId) {
                fileNamePrefix = 'Proposal Revision Monitoring Report of Multi-disciplinary Education and Research';
              } else if (this.componentId == this.sharedService.genderComponentId) {
                fileNamePrefix = 'Proposal Revision Monitoring Report of Gender Inclusion & Equity Initiatives';
              } else if (this.componentId == this.sharedService.nmdcComponentId) {
                fileNamePrefix = 'Proposal Revision Monitoring Report of New Model Degree Colleges';
              }
