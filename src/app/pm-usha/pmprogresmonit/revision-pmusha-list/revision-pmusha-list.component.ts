...
@Component({
  selector: 'cfs-revision-pmusha-list',
  templateUrl: './revision-pmusha-list.component.html',
  styleUrls: ['./revision-pmusha-list.component.scss']
})
export class RevisionPmushaListComponent implements OnInit {
...
public routers: typeof routes = routes;
  ...
  datailsList(ele: any): void {
    ...
    sessionStorage.setItem('filteredComponentId', this.componentId)
      if (ele.componentId === this.sharedService.collegeComponentId || ele.componentId === this.sharedService.universityComponentId) {
        this.router.navigate([this.routers.viewStrengthenRevision, ele.componentId])
      }
      else if (ele.componentId === this.sharedService.meruComponentId) {
        this.router.navigate([this.routers.viewMeruRevision, ele.componentId])
      }
      else if (ele.componentId == this.sharedService.nmdcComponentId){
        this.router.navigate([this.routers.viewNMDCRevision, ele.componentId])
      }
      else if (ele.componentId == this.sharedService.genderComponentId) {
        this.router.navigate([this.routers.viewEquityRevision, ele.componentId])
      }
  }
  ...
  collegeList() {
    ...
    let payload = null
    if (this.revision === this.sharedService.revPrposal ) {
      payload = {
    ...
    this.getService.getfinalSubmitProposal(payload).subscribe((res) => {
      ...
      if ((this.userTypeId === this.sharedService.userTypeList['0'].id || this.userTypeId === this.sharedService.userTypeList['6'].id || this.userTypeId === this.sharedService.userTypeList['7'].id) && this.revision === this.sharedService.revPrposal) {
      ...
      if (this.collegeListData.length != 0) {
          ...
          if (this.componentId == this.sharedService.collegeComponentId) {
            let custom_data = this.collegeListData.map((item, index) => ({
              ...
              'Submitted On': item.proposalRevisionFinalSubmissionDateTimeInString,
              'Total Cost': +item.revisedTotalCost ? +item.revisedTotalCost : +item.totalCost,
              'SAA Status': item?.proposalRevisionApprovedBySaa ? "Approved" : "Pending",
              'Esign Status': item?.revisedProposalForwardedtoNpd ? "Done" : "Pending",
            }))
             let fileNamePrefix = '';
              if (this.componentId == this.sharedService.collegeComponentId) {
